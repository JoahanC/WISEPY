...
def robust_avg(measurement,sigmas):
    ...
    if mean != len(sigmas):
        ...
        m_min = min(measurement)
        m_max = max(measurement)
        xc = (m_min + m_max) / 2.
        dmax = m_max - m_min
        if dmax <= 0:
            mu = xc
            ...
            for i in range(n):
                Y[i]=0
                for j in range(mean):
                    E = abs((measurement[j] - xc - newdx[i])/sigmas[j])
                    if E > emx:
                        CS = 2 * emx * E - emx**2
                    else:
                        CS = E**2
            ...
            while ddy <= 0 and D < dmax:
                ...
                for i in range(3):
                    ...
                    for j in range(mean):
                        E = abs((measurement[j] - xc - T)/sigmas[j])
                        if E > emx:
                            CS = 2 * emx * E - emx ** 2
                        else:
                            CS = E ** 2
            ...
            rcs = 0
            for j in range(mean):
                E = abs((measurement[j] - mu) / sigmas[j])
                if E > emx:
                    CS = 2 * emx * E - emx ** 2
                else:
                    CS = E ** 2
...
def get_distance(name, mjd):
    """
    Returns the geocentric distance for an object at a given time.
    Example: https://ssd.jpl.nasa.gov/horizons_batch.cgi?batch=1&TABLE_TYPE='OBSERVER'&COMMAND='155140'&CENTER='500'&REF_SYSTEM=J2000&START_TIME=JD2455400.5&STOP_TIME=JD2455400.6&STEP_SIZE=1d&QUANTITIES='20'&CSV_FORMAT='YES'
    Arguments: name (str) -- the object name
               mjd (float) -- the time in modified julian days
    Returns: (float) -- the geocentric distance
    """

    # Convert time format to Julian days
    jd = mjd + 2400000.5
    jd_start = str(jd)
    jd_stop = str(jd + 0.1)

    #note, single-quotes have to be converted to %27 for horizons to parse this correctly
    call_line = "https://ssd.jpl.nasa.gov/horizons_batch.cgi?batch=1&TABLE_TYPE=%27OBSERVER%27&COMMAND=%27" + name + "%27&CENTER=%27500@wise%27&REF_SYSTEM=J2000&START_TIME=JD" + jd_start + "&STOP_TIME=JD" + jd_stop + "&STEP_SIZE=1d&QUANTITIES=%2720%27&CSV_FORMAT=%27YES%27"
    
    horizons_ret = urlopen(call_line).readlines()
    for i in range(len(horizons_ret)):
        if "$$SOE" in str(horizons_ret[i]):
            break
    datum = horizons_ret[i + 1]
    return float(str(datum).split(',')[3])    
...
for inline in infs:
    ...
    for e in epochs:
        ...
        obs_dist = get_distance(horizon_name, midtime)
        ...
        for band in ["w1", "w2", "w3", "w4"]:
            if band + 'flux' in e_data.columns and "_3b" not in inline:
                ...
                try:
                    measurement = [meas_masked[i] for i in range(len(meas_masked)) if not (meas_masked.mask[i] or sig_masked.mask[i])]
                    ...
                    measurement = meas_masked
                    sigs = sig_masked

                (meanflux, meanferr, rchi) = robust_avg(measurement, sigs)
                ...
                if len(measurement) > 1:
                    meansig = rsig * (max(1, rchi / (len(measurement) - 1))) ** 0.5
                else:
                    meansig = rsig
                ...
                if len(sigs) > 0:
                    ...
                    (meanmag,meansig,rchi) = robust_avg(measurement, sigs)
                    meansig = max(meansig, 0.03) #set minimum mag error based on repeatability
